<div id="secondary-nav">
	<ul>
		<li><a href="#coffee">暗中咖啡</a></li>
		<li><a href="#party">暗中派对</a></li>
		<li><a href="#music">暗中音乐剧</a></li>
		<li><a href="#dinner">暗中夜宴</a></li>
		<li><a href="#adventure">暗中探险</a></li>
	</ul>
</div>
<div id="content">
	<div class="main">
		<div class="section" id="coffee">
			<div class="title">暗中咖啡</div>
			<div class="article">
				<img src="<?php echo get_template_directory_uri()?>/images/pages/events-1.jpg" width="100%" />
				<p class="top">暗中咖啡是黑暗中对话系列团建活动。我们邀请有挑战精神与创新意识的企业团队或学生群体进入一间完全黑暗的咖啡馆。在未来的两个小时内，参与者们将会关闭视觉，继而打开其他的感官，与伙伴们一起面对一系列未知的挑战，并在此过程中重新认识自我，发掘自身潜力。</p>
				<p>在完全黑暗的环境中，品咖啡，鉴美食。暗中咖啡帮你开启你的味觉、嗅觉、触觉、听觉……或奇妙或真诚或紧张的暗中游戏，让你真正在黑暗中打开身上的每一处感官体验。</p>
				<p>跳出嘈杂繁忙的都市生活，只想与你在黑暗里静静地喝一杯咖啡。也许你喝过九十九种不同的咖啡，却独缺黑暗里的这一杯。</p>
			</div>
			<div class="clear"></div>
		</div>
		<div class="section" id="party">
			<div class="title">暗中派对</div>
			<div class="article">
				<p>暗中派对是黑暗中对话为具有派对精神的团队推出的系列活动。作为派对达人的你可能参加过无数种派对，那必须要尝试一下这个黑暗派对。
				<p class="secondary">
					派对游戏、派对音乐、派对美食……这些你如此熟悉的元素将在黑暗中怎么被引爆；走心交流、搞怪语录、悄悄密语……这些你憧憬的交流如何在黑暗中沟通无阻。如果不用视觉沟通，调用其他感官体验，你们的默契指数是否还能再次上升。<br/>
					最好玩的是，你永远不知道黑暗中会发生什么……<br/>
					这样的派对，你来吗？<br/>
				</p>
				<p class="bottom secondary">黑暗特别时刻，即使我们隐藏其中，彼此却如此靠近。</p>
				<img src="<?php echo get_template_directory_uri()?>/images/pages/events-2.jpg" width="100%" />
			</div>
			<div class="clear"></div>
		</div>
		<div class="section" id="music">
			<div class="title wider">暗中音乐剧</div>
			<div class="article">
				<img src="<?php echo get_template_directory_uri()?>/images/pages/events-3.jpg" width="100%" />
				<p class="top">
					这是一场前所未有的观剧体验，你们将在一个完全黑暗的剧场中通过听觉、触觉、嗅觉、多元的感官体验零距离接近表演者，“身临其境”的体验无法预知的剧情。
					你，不仅仅是观众，<br/>
					你将成为音乐剧的一部分！<br/>
				</p>
				<p class="bottom secondary">穿越时空来到的暗夜、永远被神秘气息笼罩的夜光宝盒、一艘穿越在风浪中的“夜光号”、未曾谋面的黑暗船队、超乎想象的live乐队、深入腹地的探险者……这会是一场怎样颠覆想象的音乐剧；航行中的风暴，探险者们要怎么利用自己巧妙的智慧化解，又要怎么与船队配合顺利找到夜光宝盒？在黑暗中，所有人的距离将会怎样被无限拉近，黑暗里的一切未知引发你的无限想象，你的参与，赋予这部音乐剧的完整。</p>
				<p class="bottom secondary">这样的暗中音乐剧，你敢来“黑航”吗？</p>	
			</div>
			<div class="clear"></div>
		</div>
		<div class="section" id="dinner">
			<div class="title">暗中夜宴</div>
			<div class="article">
				<p>暗中夜宴是黑暗中对话系列活动。我们将邀请乐于创新、对生活品质有所追求的企业团队和学生团体共赴夜宴。在这里，你将放下平日里对视觉的依赖，随心自由地与来自各方的朋友交流畅谈，体验一场与众不同的夜宴。</p>
				<p class="secondary">暗中夜宴根据顾客们对食物的不同喜好，定制了私人菜单。素食主义者、非素食主义者都可以参加，对特定菜品的特殊要求我们都会加以安排。</p>
				<p class="secondary">也许你尝遍了大江南北山珍海味，但你一定从未品尝过我们的暗中料理。放在你面前的美食，将第一次以神秘未知的形式呈现于你。在这里，你将被迫抛弃视觉，仅用味觉、嗅觉与触觉用心感受食物的真味。</p>
				<img src="<?php echo get_template_directory_uri()?>/images/pages/events-4.jpg" width="100%" />
			</div>			
			<div class="clear"></div>
		</div>
		<div class="section" id="adventure">
			<div class="title">暗中探险</div>
			<div class="article">
				<p>暗中探险是黑暗中对话黑暗拓展类活动之一，适合有冒险精神的企业团队与学生群体的素质拓展活动。我们打造了完全黑暗的空间，辅以拟真的游戏场景效果，在神秘的真人NPC辅助下，参与者过关斩将完成一系列“不可能完成”的任务。</p>
				<p class="secondary">在寻找“达尔克文明”的黑暗旅途中，探险者们会帮助达尔克村的原住民完成一系列任务。其中包括：寻找布莱特药丸、寻找裴德多的进口宠物、绘制达尔克村地图等任务。</p>
				<p class="secondary">钢筋水泥之外、神秘密林之间，两个小时紧张刺激的探险之旅，将会带你领略失落古文明的魅力，同时提升探险者们的自我觉察以及团队配合的能力。</p>
				<img src="<?php echo get_template_directory_uri()?>/images/pages/events-5.jpg" width="100%" />
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</div>
</div>